<?php
// assy.php - Form Input & Management Transaksi Consumption ASSY (Tarik stok dari: stok_pp, stok_he, stok_piping, stok_injection)
...
$allowed_roles = ['PRESS', 'PAINTING', 'HEPI', 'INJECTION', 'PRODUCTION', 'WAREHOUSE', 'ASSY'];
...
// Active Area Filter (Default: PAINTING)
$selectedArea  = $_GET['area'] ?? 'PAINTING';

switch ($selectedArea) {
    case 'HE':
        $filteredParts = $parts_he;
        $targetTable   = 'stok_he';
        $qtyColumn     = 'qty_he';
        break;
    case 'PIPING':
        $filteredParts = $parts_piping;
        $targetTable   = 'stok_piping';
        $qtyColumn     = 'qty_piping';
        break;
    case 'INJECTION':
        $filteredParts = $parts_injection;
        $targetTable   = 'stok_injection';
        $qtyColumn     = 'qty_inj';
        break;
    case 'PAINTING':
    default:
        $selectedArea  = 'PAINTING';
        $filteredParts = $parts_paint;
        $targetTable   = 'stok_pp';
        $qtyColumn     = 'qty_paint';
        break;
}
...
// =========================================================================
// 2. HANDLE ACTION BATCH SAVE, EDIT, & DELETE (CONSUMPTION ASSY)
// =========================================================================
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {

    // --- A. BATCH INPUT ASSY (OUT/CONSUMPTION) ---
    if ($_POST['action'] === 'save_batch_assy') {
        $parts          = $_POST['parts'] ?? [];
        ...
        switch ($area) {
            case 'HE':
                $currentTable  = 'stok_he';
                $currentQtyCol = 'qty_he';
                break;
            case 'PIPING':
                $currentTable  = 'stok_piping';
                $currentQtyCol = 'qty_piping';
                break;
            case 'INJECTION':
                $currentTable  = 'stok_injection';
                $currentQtyCol = 'qty_inj';
                break;
            case 'PAINTING':
            default:
                $currentTable  = 'stok_pp';
                $currentQtyCol = 'qty_paint';
                break;
        }
        ...
        try {
            ...
            // Mengurangi stok pada tabel target
            $sqlStok = "UPDATE {$currentTable} 
                        SET {$currentQtyCol} = {$currentQtyCol} - :qty 
                        WHERE part_code = :part_code";
            $stmtStok = $pdo->prepare($sqlStok);

            // Mencatat log transaksi (Type: OUT)
            $sqlLog = "INSERT INTO stock_transactions (user_id, role, part_code, source_table, transaction_type, qty, shift, production_date, created_at) 
                       VALUES (:user_id, 'ASSY', :part_code, :source_table, 'OUT', :qty, :shift, :prod_date, :created_at)";
            ...
            foreach ($parts as $item) {
                ...
                $stmtActivity->execute([
                    ':user_id'      => $_SESSION['user_id'] ?? 1,
                    ':action'       => 'INSERT',
                    ':description'  => "Pemakaian ASSY [Area {$area} | Shift {$selectedShift} | Tgl: {$selectedDate}]: {$part_code} ({$part_name}) Qty: {$qty}",
            ...
            if ($inserted_count > 0) {
                $_SESSION['success'] = "Berhasil memotong stok $inserted_count item Part $area (Shift $selectedShift - Tgl $selectedDate)";
            } else {
                $_SESSION['error'] = "Tidak ada item yang diproses. Masukkan Qty lebih dari 0.";
            }
        } catch (Exception $e) {
            if ($pdo->inTransaction()) $pdo->rollBack();
            $_SESSION['error'] = "Gagal menyimpan data transaksi: " . $e->getMessage();
        }

        header("Location: index.php?page=assy&area=" . $area);
        exit();
    }
    ...
    if ($_POST['action'] === 'edit_transaction') {
            ...
            if ($oldTrx) {
                ...
                switch ($srcTable) {
                    case 'stok_he':
                        ...
                        break;
                    case 'stok_piping':
                        $qtyColumnTrx = 'qty_piping';
                        break;
                    case 'stok_injection':
                        $qtyColumnTrx = 'qty_inj';
                        break;
                    case 'stok_pp':
                    default:
                        $qtyColumnTrx = 'qty_paint';
                        break;
                }

                // Pengurangan selisih stok (karena OUT, jika Qty baru lebih besar, stok fisik berkurang)
                ...
                $stmtActivity->execute([
                    ':user_id'      => $_SESSION['user_id'] ?? 1,
                    ':action'       => 'UPDATE',
                    ':description'  => "Edit Trx ASSY #{$id_trx} ({$oldTrx['part_code']}) -> Qty: {$new_qty}, Shift: {$new_shift}, Tgl: {$new_date}",
                    ':target_table' => 'stock_transactions',
                ...
                $pdo->commit();
                $_SESSION['success'] = "Transaksi ASSY berhasil diperbarui!";
            }
        } catch (Exception $e) {
            if ($pdo->inTransaction()) $pdo->rollBack();
        ...
        header("Location: index.php?page=assy&area=" . $selectedArea);
        exit();
    }
    ...
    if ($_POST['action'] === 'delete_transaction') {
        ...
        try {
            ...
            if ($oldTrx) {
                ...
                // Mengembalikan stok karena pemakaian dihapus
                ...
                $stmtActivity->execute([
                    ':user_id'      => $_SESSION['user_id'] ?? 1,
                    ':action'       => 'DELETE',
                    ':description'  => "Hapus Trx ASSY #{$id_trx} ({$oldTrx['part_code']}) Qty: {$oldTrx['qty']}",
                    ':target_table' => 'stock_transactions',
                    ':part_code'    => $oldTrx['part_code'],
                    ':old_data'     => json_encode(['qty' => $oldTrx['qty'], 'shift' => $oldTrx['shift'], 'production_date' => $oldTrx['production_date']]),
                    ':new_data'     => null,
...
WHERE source_table = ? AND transaction_type = 'OUT'
GROUP BY part_code";
...
// C. History Transaksi ASSY Terbaru (Pengambilan stok / OUT)
$queryHistory = "SELECT t.*, u.username 
                 FROM stock_transactions t
                 LEFT JOIN users u ON t.user_id = u.id
                 WHERE t.source_table = :source_table AND t.transaction_type = 'OUT'
                 ORDER BY t.id DESC LIMIT 20";
$stmtHistory = $pdo->prepare($queryHistory);
$stmtHistory->execute([':source_table' => $targetTable]);
$histories   = $stmtHistory->fetchAll();

// Buat array map untuk mempermudah pencarian nama part berdasarkan code
$all_parts = array_merge($parts_paint, $parts_he, $parts_piping, $parts_injection);
$partNameMap = [];
...
<div class="container-fluid py-3 px-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <div class="btn-group" role="group">
            <a href="index.php?page=assy&area=PAINTING" class="btn btn-sm <?= $selectedArea === 'PAINTING' ? 'btn-primary' : 'btn-outline-primary' ?> fw-bold" style="width: 110px;">
                Painting Part
            </a>
            <a href="index.php?page=assy&area=HE" class="btn btn-sm <?= $selectedArea === 'HE' ? 'btn-primary' : 'btn-outline-primary' ?> fw-bold" style="width: 110px;">
                HE Part
            </a>
            <a href="index.php?page=assy&area=PIPING" class="btn btn-sm <?= $selectedArea === 'PIPING' ? 'btn-primary' : 'btn-outline-primary' ?> fw-bold" style="width: 110px; ">
                Piping Part
            </a>
            <a href="index.php?page=assy&area=INJECTION" class="btn btn-sm <?= $selectedArea === 'INJECTION' ? 'btn-primary' : 'btn-outline-primary' ?> fw-bold" style="width: 110px;">
                Injection Part
            </a>
        </div>

        <!-- Kelompok Tombol Aksi di Kanan -->
        <div class="d-flex gap-2">
            <!-- Tombol Trigger Modal Upload Plan -->
            <button type="button" class="btn btn-sm btn-success fw-bold px-2 py-1" style="font-size: 0.75rem;" data-bs-toggle="modal" data-bs-target="#uploadPlanModal">
                Upload Plan Excel
            </button>

            <!-- Tombol Laporan Pemakaian Assy -->
            <button type="button" class="btn btn-sm btn-primary fw-bold px-2 py-1" style="font-size: 0.75rem;" data-bs-toggle="modal" data-bs-target="#reportPhotoModal">
                Laporan Pemakaian ASSY
            </button>
        </div>
    </div>
    ...
    <?php if (isset($_GET['status']) && $_GET['page'] == 'assy'): ?>
        <?php if ($_GET['status'] == 'success'): ?>
            <div class="alert alert-success alert-dismissible fade show small" role="alert">
                <strong>Berhasil!</strong> <?= htmlspecialchars($_GET['count'] ?? 0) ?> data planning di-import.
                <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
            </div>
        ...
        <div class="col-lg-5 col-md-12">
            <div class="card shadow-sm border-0">
                <div class="card-header bg-dark text-white d-flex justify-content-between align-items-center py-2">
                    <span class="fw-bold" style="font-size: 0.9rem;">Input Pemakaian (<?= $selectedArea ?>)</span>
        ...
        <div class="col-lg-7 col-md-12">
            <div class="card shadow-sm border-0">
                <div class="card-header bg-dark text-white fw-bold py-2 d-flex justify-content-between align-items-center" style="font-size: 0.9rem;">
                    <span>Riwayat Pemakaian Stok (<?= $selectedArea ?>)</span>
                </div>
                    ...
                    <div class="table-responsive" style="max-height: 308px; overflow-y: auto;">
                        <table class="table table-hover align-middle text-center mb-0" style="font-size: 0.8rem;">
                            <thead class="table-light sticky-top">
                                <tr>
                                    <th style="width: 120px;" class="text-start">Waktu Input</th>
                                    <th style="width: 100px;">Tgl Prod</th>
                                    <th style="width: 60px;">Shift</th>
                                    <th class="text-start">Part Code</th>
                                    <th class="text-start">Part Name</th> <!-- Kolom Baru -->
                                    <th>Qty Out</th>
                                    <th>Operator</th>
                            ...
                            <tbody>
                                <?php if (count($histories) > 0): ?>
                                    ...
                                    ?>
                                        ...
                                        <!-- MODAL EDIT & DELETE UNTUK SETIAP BARIS -->
                                        <div class="modal fade" id="editModal<?= $tr['id'] ?>" tabindex="-1" aria-hidden="th">
                                            <div class="modal-dialog modal-dialog-centered">
                                                <div class="modal-content">
                                                    <form method="POST" action="index.php?page=assy&area=<?= $selectedArea ?>">
                                                        <div class="modal-header bg-warning text-dark py-2">
                                                            <h6 class="modal-title fw-bold">Edit Transaksi #<?= $tr['id'] ?></h6>
                                                        ...
                                                        <div class="modal-body">
                                                            ...
                                                            <div class="mb-2">
                                                                ...
                                                                <input type="text" class="form-control form-control-sm" value="<?= $tr['part_code'] ?>" disabled>
                                                            </div>
                                                            <div class="mb-2">
                                        ...
                                        <div class="modal fade" id="deleteModal<?= $tr['id'] ?>" tabindex="-1" aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-centered">
                                                <div class="modal-content">
                                                    <form method="POST" action="index.php?page=assy&area=<?= $selectedArea ?>">
                                                        <div class="modal-header bg-danger text-white py-2">
                                                            <h6 class="modal-title fw-bold">Konfirmasi Hapus Transaksi #<?= $tr['id'] ?></h6>
                                                            <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                                                        </div>
                                                        <div class="modal-body">
                                                            <input type="hidden" name="action" value="delete_transaction">
                                                            <input type="hidden" name="id_transaction" value="<?= $tr['id'] ?>">
                                                            <p class="mb-1">Apakah Anda yakin ingin menghapus transaksi part <b><?= $tr['part_code'] ?></b> dengan Qty <b><?= $tr['qty'] ?></b>?</p>
                                                            <small class="text-danger fw-bold">Stok fisik akan dikembalikan secara otomatis.</small>
                                                        </div>
                                                        <div class="modal-footer py-2">
                                                            <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Batal</button>
                                ...
                                <?php else: ?>
                                    <tr>
                                        <td colspan="8" class="text-muted py-4">Belum ada riwayat pemakaian untuk Area <?= $selectedArea ?>.</td>
                                    </tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
                            ...
                            <tbody>
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td class="fw-bold text-danger"><?= number_format((int)($trx['daily'] ?? 0)) ?></td>
        ...
        <div class="modal-content">
            <form method="POST" action="index.php?page=assy&area=<?= $selectedArea ?>">
                <div class="modal-header bg-warning text-white py-2">
                    <h6 class="modal-title fw-bold">Input Susulan (<?= $selectedArea ?>)</h6>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <input type="hidden" name="action" value="save_batch_assy">
        ...
        <div class="modal-content border-0 shadow-lg">
            <div class="modal-header bg-primary text-white py-3">
                <div>
                    <h5 class="modal-title fw-bold mb-0">
                        LAPORAN PEMAKAIAN STOK - <?= $selectedArea ?>
                    </h5>
                </div>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body p-4 bg-white" id="printableReportArea">
                <div class="d-flex justify-content-between align-items-center border-bottom pb-3 mb-3">
                    <div>
                        <h4 class="fw-bold text-dark mb-0">LAPORAN PEMAKAIAN ASSY - AREA <?= $selectedArea ?></h4>
                    </div>
                    <div class="text-end">
                    ...
                    <table class="table table-bordered align-middle text-center mb-0" style="font-size: 0.9rem;">
                        <thead class="table-dark text-uppercase fs-6">
                            <tr>
                                ...
                                <th style="width: 150px;" class="bg-danger text-white">Total Daily</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td class="fw-bold fs-6 text-danger bg-light"><?= number_format($daily) ?></td>
                                </tr>
                            <?php endforeach; ?>

                            <?php if (!$hasData): ?>
                                <tr>
                                    <td colspan="7" class="text-muted py-4">Belum ada transaksi pemakaian untuk hari ini.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                        <?php if ($hasData): ?>
                            <tfoot class="table-secondary fw-bold fs-6">
                                <tr>
                                    <td colspan="3" class="text-end py-2">TOTAL PEMAKAIAN:</td>
                                    ...
                                    <td class="text-danger fs-6 bg-warning bg-opacity-25"><?= number_format($grand_daily) ?></td>
                                </tr>
                            </tfoot>
                        <?php endif; ?>
                    </table>
...
<!-- MODAL UPLOAD PLAN EXCEL -->
<div class="modal fade" id="uploadPlanModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg">
            <form action="process.php" method="POST" enctype="multipart/form-data">
                <div class="modal-header bg-success text-white py-2">
                    <h6 class="modal-title fw-bold">Upload File Planning Excel (.xlsm / .xlsx)</h6>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body p-3">
                    <div class="mb-3">
                        <label class="form-label small fw-bold">Pilih File Excel Macro / Workbook</label>
                        <input class="form-control form-control-sm" type="file" name="excel_file" accept=".xlsx, .xlsm, .xls" required>
                        <div class="form-text" style="font-size: 0.75rem;">Format yang didukung: .xlsx, .xlsm, .xls</div>
                    </div>
                </div>
                <div class="modal-footer bg-light py-2">
                    <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Tutup</button>
                    <button type="submit" name="upload" class="btn btn-success btn-sm fw-bold">Import Planning</button>
                </div>
            </form>
        </div>
    </div>
